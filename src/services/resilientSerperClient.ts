...
export class ResilientSerperClient extends SerperClient {
   ...
   */
  private searchResultsToQuotes(results: ISerperSearchResult[], query: string): IQuote[] {
    const quotes: IQuote[] = []
   ...
   */
  private quotesToSearchResults(quotes: IQuote[]): ISerperSearchResult[] {
  ...
  private registerHealthChecks(): void {
    ...
    if (this.resilientConfig.enableCircuitBreaker) {
      healthCheckManager.register(
        ...
        createCircuitBreakerHealthCheck('serper-api', () => this.apiCircuitBreaker.getStats())
      )
    }
    ...
    if (this.resilientConfig.apiKey) {
      healthCheckManager.register(
        'serper-api',
        createSerperHealthCheck(this.resilientConfig.apiKey, this.resilientConfig.baseUrl)
      )
    }
  }
}
