...
export class TemplateRenderer {
  ...
  static async render(
    ...
    try {
      ...
      let output = this.substituteVariables(
        template.content,
        variables,
        context.options?.formatters
      )
  ...
  private static applyTransformer(content: string, processor: IPostProcessor): string {
    ...
    switch (name) {
      ...
      case 'number-lines':
        ...
        return numberLines
          .map((line: string, index: number) => `${startNum + index}. ${line}`)
          .join('\n')
  ...
  private static applyEnricher(
    ...
    switch (name) {
      ...
      case 'add-attribution':
        const attribution =
          options?.['text'] || `Generated using ${template.metadata.name} template`
        return `${content}\n\n--- ${attribution} ---`
