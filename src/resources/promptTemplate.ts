...
const promptTemplates: Record<string, PromptTemplateData> = {
  default: {
    ...
    template:
      'Find {numberOfQuotes} quotes from {person}{topic}. Return them in a structured format with proper attribution.',
    variables: {
      person: {
        description:
          'The person whose quotes you want to retrieve. Can be any famous or notable individual.',
        required: true,
        type: 'string',
    ...
    instructions:
      'Replace variables in curly braces with your desired values. The topic variable should include " about " prefix when used.',
    examples: [
      {
        name: 'Basic request without topic',
        description: 'Retrieve quotes without topic filtering',
        request:
          'Find 3 quotes from Albert Einstein. Return them in a structured format with proper attribution.',
        expectedFormat:
      ...
      },
      {
        ...
        request:
          'Find 3 quotes from Maya Angelou about courage. Return them in a structured format with proper attribution.',
        expectedFormat:
          '1. "Courage is the most important of all the virtues..." - Maya Angelou\n2. "One isn\'t necessarily born with courage..." - Maya Angelou\n3. "Without courage we cannot practice any other virtue..." - Maya Angelou',
      },
    ],
    tips: [
  ...
  research: {
    ...
    template:
      'Find {numberOfQuotes} verified quotes from {person} about {topic}. Include source, date, and context for each quote. Prioritize primary sources.',
    variables: {
      person: {
    ...
    instructions:
      'This template is designed for academic research. It requests verified quotes with full source attribution.',
    examples: [
      {
        name: 'Academic research query',
        description: 'Retrieve verified quotes with full attribution',
        request:
          'Find 5 verified quotes from Charles Darwin about evolution. Include source, date, and context for each quote. Prioritize primary sources.',
        expectedFormat:
          '1. "It is not the strongest of the species that survives..." - Charles Darwin\n   Source: On the Origin of Species (1859), Chapter 4\n   Context: Discussing natural selection and adaptation\n   Date: November 24, 1859',
      },
    ],
    tips: [
  ...
  creative: {
    ...
    template:
      'Find {numberOfQuotes} inspirational quotes {sources} about {theme}. Focus on quotes that evoke emotion and creativity. Group by mood or tone.',
    variables: {
      numberOfQuotes: {
      ...
      sources: {
        ...
        examples: [
          ...
          'from novelists',
          'from musicians',
          'from various artists and writers',
        ],
        constraints: {},
      },
      theme: {
    ...
    examples: [
      {
        ...
        request:
          'Find 7 inspirational quotes from various artists and writers about creativity and imagination. Focus on quotes that evoke emotion and creativity. Group by mood or tone.',
        expectedFormat:
...
export async function promptTemplateHandler(uri: string): Promise<string> {
  ...
  if (!template) {
    ...
    throw new Error(
      `Template '${templateId}' not found. Available templates: ${availableTemplates.join(', ')}`
    )
  }
