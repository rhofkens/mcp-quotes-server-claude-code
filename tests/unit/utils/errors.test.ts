...
describe('Error Handling Utilities', () => {
  describe('BaseError', () => {
    it('should create a base error with all properties', () => {
      const error = new BaseError('Test error message', ErrorCode.INTERNAL_ERROR, 500, {
        foo: 'bar',
      })
    ...
    it('should convert to JSON representation', () => {
      const error = new BaseError('Test error', ErrorCode.INTERNAL_ERROR, 500, { test: true })
  ...
  describe('ValidationError', () => {
    ...
    it('should format user message with field name', () => {
      ...
      expect(error.getUserMessage()).toBe(
        "Invalid value for field 'userEmail': must be a valid email. Please refer to the prompt template resource for the correct format."
      )
    })
    ...
    it('should format user message without field name', () => {
      ...
      expect(error.getUserMessage()).toBe(
        'Validation error: Invalid input format. Please check that all required parameters are provided correctly.'
      )
    })
  })
  ...
  describe('APIError', () => {
    it('should create API error with service name', () => {
      const error = new APIError('Request failed', ErrorCode.API_ERROR, 'SerperAPI')
    ...
    it('should provide user-friendly messages for API errors', () => {
      ...
      expect(rateLimit.getUserMessage()).toBe(
        'Rate limit exceeded. Please wait a few minutes before making more requests. Consider reducing the number of quotes requested.'
      )

      const unauthorized = new APIError('Unauthorized', ErrorCode.API_UNAUTHORIZED)
      expect(unauthorized.getUserMessage()).toBe(
      ...
      const generic = new APIError('Generic error')
      expect(generic.getUserMessage()).toBe(
  ...
  describe('ConfigError', () => {
    ...
    it('should format user message without variable name', () => {
      ...
      expect(error.getUserMessage()).toBe(
        'Configuration error: Please check your environment settings'
      )
    })
  })
  ...
  describe('ErrorFormatter', () => {
    ...
    it('should format error for user display', () => {
      ...
      expect(ErrorFormatter.formatForUser(baseError)).toBe(
        "Invalid value for field 'email': Invalid input. The email field is not a standard field for the quote tool. Please check the available fields."
      )

      const regularError = new Error('Technical details')
      expect(ErrorFormatter.formatForUser(regularError)).toBe(
